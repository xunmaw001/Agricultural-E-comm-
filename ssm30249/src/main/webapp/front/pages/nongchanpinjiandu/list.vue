...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"width":"100%","padding":"0","position":"relative","background":"#f6f6f6","height":"auto"}'>
			...
			<view class="uni-product-list" :style='{"padding":"100rpx 24rpx 24rpx","margin":"24rpx 0 0","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20221220/8fb6b4f30229439db2e15009b4e57f85.gif) no-repeat right top / 240rpx,url(http://codegen.caihongy.cn/20221220/8fb6b4f30229439db2e15009b4e57f85.gif) no-repeat left center / 240rpx,url(http://codegen.caihongy.cn/20221219/53c3a93f62624145a4483eea1ef21d2d.png) no-repeat center top,linear-gradient(0deg, rgba(246,246,246,1) 0%, rgba(103,197,254,1) 80%, rgba(6,159,250,1) 100%)","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"0px solid #f7de91","boxShadow":"0 8rpx 6rpx rgba(0,0,0,.1),inset 0px 0px 0px 0px #ffef92","padding":"20rpx","margin":"0 0 40rpx","backgroundColor":"rgba(255,255,255,.8)","borderRadius":"24rpx","flexWrap":"wrap","display":"flex","width":"48%","position":"relative","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","whiteSpace":"nowrap","overflow":"hidden","color":"#333","textAlign":"center","width":"96%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis","order":"2"}'>{{product.shangpinmingcheng}}</view>
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","whiteSpace":"nowrap","overflow":"hidden","color":"#333","textAlign":"center","width":"96%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis","order":"2"}'>{{product.shangpinfenlei}}</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('nongchanpinjiandu','修改')) || (!userid && isAuthFront('nongchanpinjiandu','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('nongchanpinjiandu','删除')) || (!userid && isAuthFront('nongchanpinjiandu','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 2rpx 8rpx rgba(0,0,0,.3)","color":"#333","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"radial-gradient(circle, rgba(246,246,246,1) 0%, rgba(103,197,254,1) 80%, rgba(6,159,250,1) 100%),#069ffa","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"fixed","height":"80rpx","zIndex":"9999"}' v-if="userid && isAuth('nongchanpinjiandu','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 2rpx 8rpx rgba(0,0,0,.3)","color":"#333","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"radial-gradient(circle, rgba(246,246,246,1) 0%, rgba(103,197,254,1) 80%, rgba(6,159,250,1) 100%),#069ffa","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"fixed","height":"80rpx","zIndex":"9999"}' v-if="!userid && isAuthFront('nongchanpinjiandu','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`nongchanpinjiandu`, params);
                } else {
                    params['sfsh'] = '是';
                    res = await this.$api.list(`nongchanpinjiandu`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('nongchanpinjiandu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`nongchanpinjiandu`, searchForm);
                } else {
                    res = await this.$api.list(`nongchanpinjiandu`, searchForm);
                }
				// 如果是第一页数据置空
