	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230329/eff4ea96c952463cb46b9c79d043ce56.gif) fixed,#f6f6f6"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="温度" prop="wendu">
					<el-input v-model="ruleForm.wendu" placeholder="温度" clearable  :readonly="ro.wendu"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="温度" prop="wendu">
					<el-input v-model="ruleForm.wendu" placeholder="温度" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="湿度" prop="shidu">
					<el-input v-model="ruleForm.shidu" placeholder="湿度" clearable  :readonly="ro.shidu"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="湿度" prop="shidu">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.shengzhanghuanjing" label="生长环境" prop="shengzhanghuanjing">
					<file-upload
						tip="点击上传生长环境"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.shengzhanghuanjing?ruleForm.shengzhanghuanjing:''"
						@change="shengzhanghuanjingUploadChange"
					></file-upload>
				</el-form-item> 
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shengzhanghuanjing" label="生长环境" prop="shengzhanghuanjing">
					<el-button :style='{"border":"0px solid #3ca512","cursor":"pointer","padding":"0 35px","margin":"0 20px 0 0","outline":"none","color":"#fff","borderRadius":"0px","background":"#339933","width":"auto","lineHeight":"40px","fontSize":"16px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.shengzhanghuanjing)">预览</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="!ruleForm.shengzhanghuanjing" label="生长环境" prop="shengzhanghuanjing">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="物流状况" prop="wuliuzhuangkuang">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.wuliuzhuangkuang" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.wuliuzhuangkuang" label="物流状况" prop="wuliuzhuangkuang">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.wuliuzhuangkuang"></span>
                </el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='wuliuzhuangkuang'){
							this.ruleForm.wuliuzhuangkuang = obj[o];
							this.ro.wuliuzhuangkuang = true;
							continue;
						}
				}
            ...
            this.zhuangtaiOptions = "正常,良好,较差".split(',')
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.wuliuzhuangkuang = this.ruleForm.wuliuzhuangkuang.replace(reg,'../../../ssm30249/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.shengzhanghuanjing!=null) {
		this.ruleForm.shengzhanghuanjing = this.ruleForm.shengzhanghuanjing.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `nongchanpinjiandu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.nongchanpinjianduCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shengzhanghuanjingUploadChange(fileUrls) {
	    this.ruleForm.shengzhanghuanjing = fileUrls;
    },
  }
};
</script>
