<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230329/eff4ea96c952463cb46b9c79d043ce56.gif) fixed,#f6f6f6"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="商家账号" prop="shangjiazhanghao">
					<el-select :disabled="ro.shangjiazhanghao" @change="shangjiazhanghaoChange" v-model="ruleForm.shangjiazhanghao" placeholder="请选择商家账号">
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.shangjiazhanghao" label="商家账号" prop="shangjiazhanghao">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="提醒标题" prop="tixingbiaoti">
					<el-input v-model="ruleForm.tixingbiaoti" placeholder="提醒标题" clearable  :readonly="ro.tixingbiaoti"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="提醒标题" prop="tixingbiaoti">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.tixingneirong" label="提醒内容" prop="tixingneirong">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.tixingneirong"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			shangjiazhanghaoChange () {
				this.$http({
					...
					if (data && data.code === 0) {
						if(data.data.dianpumingcheng){
							this.ruleForm.dianpumingcheng = data.data.dianpumingcheng
						}
					} else {
						this.$message.error(data.msg);
					}
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.tixingneirong = this.ruleForm.tixingneirong.replace(reg,'../../../ssm30249/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhunongxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhunongxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
